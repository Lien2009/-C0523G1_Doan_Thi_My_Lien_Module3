...
insert into instructor(`name`,birthday, salary)
 ...
 SELECT student.class_id, class.`name`, count(student.class_id) AS total_student
 FROM student
 JOIN class ON student.class_id = class.id
 GROUP BY student.class_id;
 -- 2.Tính điểm lớn nhất của mỗi các lớp
 SELECT student.class_id, class.`name`, max(student.point) AS max_point
 FROM student
 JOIN class ON student.class_id = class.id
 GROUP BY student.class_id;
 -- 3.Tình điểm trung bình  của từng lớp
 SELECT student.class_id, class.`name`, AVG(student.point) AS max_point
 FROM student
 JOIN class ON student.class_id = class.id
 GROUP BY student.class_id;
