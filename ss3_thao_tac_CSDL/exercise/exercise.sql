...
-- Lấy ra thông tin các học viên, và cho biết các học viên đang theo học lớp nào (chỉ học viên có lớp học).
select * FROM student;
SELECT * FROM class;
...
SELECT COUNT(id), `point`
FROM student
GROUP BY `point` HAVING `point` > 5
ORDER BY `point`;
-- Thông kế số lượng học sinh theo điểm lớn hơn 5 và chỉ hiện thị với số lượng>=2
SELECT COUNT(id) AS C, `point`
FROM student
GROUP BY `point` HAVING `point` > 5 AND C >= 2
ORDER BY `point`;
